...
export class AppointmentManagementComponent implements OnInit{
  ...
  onDateSelected(date: Date) {
    ...
    this.landingPageService.getListAppointmentByDoctorIdAndDateAndNotYetConfirmed(this.data).subscribe(res=>{
  ...
  handleConfirmConsultStatus(appointment: Appointment) {
    if (confirm("Đã xác nhận với bệnh nhân ?")){
      this.landingPageService.confirmConsultStatus(appointment.id).subscribe(res =>{
        this.onDateSelected(appointment.appDate)
      })
    }
  }

  handleConfirmStatus(appointment: Appointment) {
    if (confirm("Đã khám xong ?")){
      this.landingPageService.confirmStatus(appointment.id).subscribe(res =>{
        this.onDateSelected(appointment.appDate)
      })
    }
  }
